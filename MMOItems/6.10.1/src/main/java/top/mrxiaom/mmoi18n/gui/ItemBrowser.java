...
public class ItemBrowser extends net.Indyuce.mmoitems.gui.ItemBrowser implements InjectedInventory {
    ...
    public ItemBrowser(PluginMain plugin, @NotNull Navigator navigator) {
        this(plugin, navigator, null);
    }

    public ItemBrowser(PluginMain plugin, @NotNull Navigator navigator, Type type) {
    ...
    public Inventory getInventory() {
        ...
        if (type == null) {
            ...
            for (int j = min; j < Math.min(max, itemTypes.size()); j++) {
                ...
                if (meta != null) {
                    ...
                    String translated = translateType(currentType.getId());
        ...
        String typeName;
        {
            String translated = translateType(type.getId());
    ...
    public void whenClicked(InventoryClickEvent event) {
        ...
        if (MMOUtils.isMetaItem(item, false)) {
            if (has(item, "next_page")) {
                ...
                new ItemBrowser(plugin, getNavigator(), MMOItems.plugin.getTypes().get(typeId)).open();
            }
        }
        ...
        if (deleteMode) {
            ...
            if (event.getAction() == InventoryAction.PICKUP_HALF) {
                new ItemEdition(plugin, getNavigator(), MMOItems.plugin.getTemplates().getTemplate(type, id)).open();
            }
        }
    }
}
